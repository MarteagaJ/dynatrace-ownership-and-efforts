...
- **Add a sub-page** (a page only reachable by drilling into a main page, e.g. `/teams/:teamId/problems`): render `PageBreadcrumbs` from [ui/app/components/PageBreadcrumbs.tsx](ui/app/components/PageBreadcrumbs.tsx) as the first element in the page, above its `TitleBar`, with a `trail` array from the nearest main page down to the current page (last entry = current page). Don't add the page to `Header.tsx`'s nav — sub-pages are reached only via links/tiles on their parent page and the breadcrumb trail back out.
